...
| Bonus #2 — Rename / delete conversations | Completed | Long-press `MenuAnchor` on `lib/conversations/widgets/conversation_tile.dart`; rename via `ConversationsCubit.rename` (+ `ConversationsTransientError.renameFailed`); delete via `AppConversationDeleted` on `AppBloc` (+ `AppTransientError.deleteFailed`, registry dispose, active-id clear); dialogs in `lib/conversations/widgets/{rename,delete}_conversation_dialog.dart`. |
| Bonus #3 — Folders + folder CRUD + move | Completed | Two-section drawer (`lib/conversations/view/conversations_drawer.dart`) renders folders (each an `ExpansionTile` via `lib/conversations/widgets/folder_tile.dart`) above an "Uncategorized" section. `+` action creates a folder via `CreateFolderDialog`. Folder long-press menu rename/delete via `RenameFolderDialog` / `DeleteFolderDialog` (the latter names the cascading conversation count via the `deleteFolderDialogBody` plural). Conversation long-press gains a **Move to folder…** `SubmenuButton` listing every folder, "Uncategorized", and "New folder…". CRUD actions live on `ConversationsCubit` (`createFolder` / `renameFolder` / `moveConversation` / `moveConversationToNewFolder` + `ConversationsTransientError.{moveFailed,folderCreateFailed,folderRenameFailed}`). Folder delete routes through `AppFolderDeleted` on `AppBloc` (+ `AppTransientError.folderDeleteFailed`) so cascaded conversations' chat-repo entries are disposed and the active id is cleared when it falls inside the deleted folder. |

### Settled Design Choices (do not relitigate)

- **Persistence backend:** drift (over hydrated_bloc/hive). Reactive `watch()` + native cascade deletes for the bonus features. Folder delete cascades via SQL `ON DELETE CASCADE` on `Conversations.folderId`; conversation delete cascades to `Messages` the same way.
...
- **Folder delete is app-orchestrated, conversation move/folder rename/folder create are cubit-orchestrated.** Folder delete must dispose chat repositories for cascaded conversations and clear the active id when applicable, so it lives on `AppBloc` (mirrors `AppConversationDeleted`). The cascading conversation ids are computed at the call site from the cubit's current state and passed through `AppFolderDeleted` — the bloc never queries the database for them.
- **Move-to-folder includes the current folder.** The submenu lists every folder without filtering. Picking the current folder is short-circuited at the call site (`_onMoveRequested` returns early when `conversation.folderId == folderId`) — no repository round-trip.
...
- **ChatBloc lifecycle.** `ChatBloc` (`lib/chat/bloc/chat_bloc.dart`) takes `conversationId`, `ConversationsRepository`, and a `ChatRepositoryRegistry`. It subscribes to `watchMessages(conversationId)` in its constructor and emits `ChatMessagesUpdated` on every drift snapshot. `close()` cancels the subscription. There is no explicit connect/disconnect on the bloc — the `ChatRepositoryRegistry` owns connection lifecycle. The registry is provided as a peer of `ConversationsRepository` via `MultiRepositoryProvider` in `App`, so both `AppBloc` and `ChatBloc` read it from `context`.
...
- **Send paths.** Two paths converge at the same drift writes:
  - **First send (`activeConversationId == null`).** The empty-state composer fires `AppFirstMessageSubmitted(text)` to `AppBloc`, which runs `autoTitle` → `createConversation` → `appendMessage` (user) → activate id (and persist last-active metadata) → `chatRepositoryRegistry.obtain(id)` → `send`. Failures map to `AppTransientError.{persistenceFailed,connectionFailed,messageTooLarge,sendFailed}`.
