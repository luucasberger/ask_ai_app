...
| Required #2 — Conversation history | Done (pending final sweep) | All wiring landed: drift-backed persistence, AppBloc orchestration, ChatBloc per-conversation, conversations drawer, ChatPage drawer integration. Step 6 (full-suite analyze + coverage + manual smoke) is the only outstanding item. |
...
### Pending Work — Required #2

The conversation-history work is in flight. The following landed and is covered by tests at 100%:

- `packages/conversations_client/conversations_client/` — interface, entities (`Conversation`, `Folder`, `Message`), exception hierarchy.
- `packages/conversations_client/drift_conversations_client/` — drift-backed implementation. Schema includes folders + nullable `folderId` on conversations from day 1 so bonus #2/#3 are UI-only when their turn comes. ISO8601 datetime storage (`storeDateTimeAsText: true`) so UTC roundtrips correctly. Foreign keys with `ON DELETE CASCADE` enabled via `PRAGMA foreign_keys = ON`. Tests cover happy paths via `NativeDatabase.memory()` and error paths via a `LazyDatabase` whose factory throws.
- `packages/conversations_repository/` — thin pass-through over the client.
- `lib/main/bootstrap/storage.dart` — opens the file-backed drift executor at `getApplicationSupportDirectory()/ask_ai.sqlite`. All three flavor entrypoints wire `ConversationsRepository` into `App`.
- `lib/app/registry/chat_repository_registry.dart` — owns one `ChatRepository` per conversation id (Design A: per-conversation socket). Forwards each repo's `incomingMessages` to a single `onEcho` callback so echoes route correctly even when no `ChatBloc` is mounted for the destination conversation.
- `lib/app/bloc/app_bloc.dart` — owns `activeConversationId`, `streamingMessageId`, persistence of incoming echoes, and last-active resume via `app_metadata`. Exposes `obtainChatRepository(id)` and `AppBloc.autoTitle(text)` (40-char cap with ellipsis).

The next steps to land Required #2:

1. **Refactor `ChatBloc`** (`lib/chat/bloc/`) — _**Done.**_ Constructor takes `conversationId`, `ConversationsRepository`, and a `ChatRepositoryProvider` typedef (`Future<ChatRepository> Function(String)`); the bloc subscribes to `watchMessages(conversationId)` in its constructor and emits `ChatMessagesUpdated` on every drift emission. `awaitingResponse` is derived on `ChatState` (`messages.last.role == user`). `_onMessageSubmitted` persists the user message via `appendMessage` first, then obtains the per-conversation `ChatRepository` and calls `send`; failures map to `ChatTransientError.{persistenceFailed,connectionFailed,messageTooLarge,sendFailed}`. Removed: `ChatStarted` / `ChatBackendMessageReceived` / `ChatStreamingCompleted` events, `ChatStatus` enum, the `status` / `streamingMessageId` / `awaitingResponse` state fields, `canSend` / `isResponseInFlight` getters. Added: `ChatMessagesUpdated`, `ChatTransientErrorCleared`, `ChatTransientError.persistenceFailed`. Bloc tests (`bloc/{chat_bloc,chat_event,chat_state}_test.dart`) rewritten and at 100% coverage on the bloc files. **Consumers (`chat_page.dart`, `chat_composer.dart`, `chat_bubble.dart`) and their widget tests still reference the old `ChatBloc` shape and the old `Message` from `lib/chat/model/message.dart`; they are knowingly broken until step 2 retires the old `Message` and step 4 rewires the view.**
2. **Move `Message` out of `lib/chat/model/`** — _**Done.**_ Deleted `lib/chat/model/message.dart` (and the now-empty `lib/chat/model/` directory) plus `test/chat/model/message_test.dart`. Dropped the `model/message.dart` re-export from `lib/chat/chat.dart`. `lib/chat/widgets/chat_bubble.dart` and `test/chat/widgets/chat_bubble_test.dart` now import `Message`/`MessageRole` from `package:conversations_repository/conversations_repository.dart` and supply `conversationId` + `sentAt` at construction. `test/chat/view/chat_page_test.dart` had its stale `chat/model/message.dart` import retargeted as well — the file remains broken on the bloc-state references from step 1 and will be fully rewritten in step 4.
3. **Build the conversations drawer** (`lib/conversations/`) — _**Done.**_ New feature folder with `cubit/` (`ConversationsCubit` subscribes to `watchConversations()` and re-emits `ConversationsState` on every drift snapshot), `widgets/conversation_tile.dart` (ListTile-backed row that exposes `onTap`; long-press deliberately not wired — bonus #2 will attach the context menu here), `view/conversations_drawer.dart` (`ConversationsDrawer` provides the cubit; `ConversationsDrawerView` renders the `Drawer` with a `FilledButton.icon` "New chat" CTA, a `Conversations` section header, the live tile list, and a localized empty state), plus `conversations.dart` barrel. The drawer accepts `activeConversationId` plus `onConversationTapped(id)` / `onNewChatTapped()` callbacks — wiring those to `AppBloc` events lives in step 4. Tests at `test/conversations/**` cover all new files at 100%. **Step 5 (localization) was rolled into this step**: `drawerNewChat`, `drawerEmpty`, `drawerSectionConversations` keys added to `lib/l10n/arb/app_{en,es}.arb`, regenerated into `lib/l10n/gen/`.
4. **Hook the drawer into `ChatPage`** — _**Done.**_ `App` widget now provides `RepositoryProvider<ConversationsRepository>` + `BlocProvider<AppBloc>(..AppStarted)`; flavor entrypoints (`main_*.dart`) pass `chatRepositoryFactory: (_) => ChatRepository(chatClient: WebSocketChatClient(...))` so the registry mints a fresh socket per conversation. `ChatPage` provides a page-scoped `ConversationsCubit` (drawer + AppBar title share it). `ChatView` switches on `AppState.activeConversationId`: empty-state body when null (composer dispatches `AppFirstMessageSubmitted` — new event/handler on `AppBloc` that runs `autoTitle` → `createConversation` → `appendMessage` → activate → `obtain` → `send`, mapping failures to the four `AppTransientError` variants), and an `_ActiveChatBody` keyed by `ValueKey(activeId)` when set (provides `ChatBloc(conversationId, conversationsRepository, chatRepositoryProvider: appBloc.obtainChatRepository)`). `_ActiveChatComposer` combines `AppBloc.streamingMessageId` (via `BlocBuilder`) with `ChatBloc.awaitingResponse` to gate `inFlight`. The drawer is wrapped in a `Builder` so `Scaffold.of(context).closeDrawer()` resolves; tile taps fire `AppConversationActivated`, the "New chat" CTA fires `AppNewConversationRequested`. Two `BlocListener`s surface transient errors as snackbars (one for `AppBloc`, one for `ChatBloc`) and fire the corresponding `*TransientErrorCleared` event after surfacing. `ChatComposer` is now callback-based (`onSubmit` + `inFlight`) and bloc-agnostic. New `chatErrorPersistenceFailed` ARB string added (en + es) for `persistenceFailed`. `pump_app.dart` now provides `BlocProvider<AppBloc>` (with a `MockBloc`-backed default for tests that don't care). All affected widget tests rewritten; suite is at 118 tests, 100% coverage, analyzer clean.
5. ~~**Localization**~~ — _Folded into step 3._
6. **Final sweep** — `fvm dart analyze`, `very_good test --coverage --min-coverage 100 --test-randomize-ordering-seed random --exclude-coverage='**/*.g.dart'`, manual smoke (run dev flavor, two conversations, mid-stream switch test, kill+relaunch).

Design choices already settled with the user (do not relitigate):
